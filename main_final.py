...
def APDP2delays(vermogen: list, dT: float) -> list:
    ...
    gesorteerde_pieken = sorted(
        piek_indexen, key=lambda i: vermogen[i], reverse=True)

    # Pak de 2 grootste pieken als τ0 en τ1
    twee_grootste_pieken = gesorteerde_pieken[:2] if len(
        gesorteerde_pieken) >= 2 else gesorteerde_pieken
...
def calculate_location(delays: list):
...
def mediaanfout(locationsx: list, locationsy: list, echt_locationsx: list, echt_locationsy: list):
    xfout = []
    yfout = []
